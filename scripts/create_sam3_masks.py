...
def run(settings_path: Path, device_override: str | None = None) -> int:
	...
	output_root = (repo_root / settings.output_folder).resolve()
	...
	for timestep_value, timestep_input_dir in selected_timesteps:
		...
		output_dir = output_root / timestep_name / "masks"
		...
		print(f"Finished {timestep_name}: {folder_processed} new, {folder_skipped} skipped")
