...
def prepare_timestep(frame_root: Path) -> dict[str, str]:
	...
	return {
		...
		"status": "ensured sparse/0 from provided sparse model",
	}
...
def main() -> int:
	parser = argparse.ArgumentParser(description="Normalize provided COLMAP sparse models into sparse/0 for selected timestep folders.")
	parser.add_argument(
		"--settings",
		type=Path,
