...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--render-mode",
        choices=("splats", "points"),
        default="splats",
        help="Render full Gaussian splats or a fast point-only preview. Default: splats",
    )
    parser.add_argument(
        "--point-radius",
        type=int,
        default=1,
        help="Point radius in pixels for --render-mode points. Default: 1",
    )
...
class GaussianSplatRenderer:
    ...
    def render(self, cloud: LoadedGaussianCloud, pose: CameraPose) -> np.ndarray:
        if self.render_mode == "points":
            return self._render_points(cloud, pose)
        if self.device == "cuda":
            return self._render_torch(cloud, pose)
        return self._render_numpy(cloud, pose)

    def _render_points(self, cloud: LoadedGaussianCloud, pose: CameraPose) -> np.ndarray:
        ...
        y = y[visible]
        z = z[visible]
        colors = np.clip(cloud.colors[visible] * 255.0, 0.0, 255.0).astype(np.uint8)

        screen_x = self.fx * x / z + self.cx
        screen_y = self.cy - self.fy * y / z
        in_frame = screen_x >= 0.0
        in_frame &= screen_x < self.width
        in_frame &= screen_y >= 0.0
        in_frame &= screen_y < self.height
        ...
        draw_order = np.argsort(z)[::-1]
        pixel_x = np.rint(screen_x[draw_order]).astype(np.int32)
        pixel_y = np.rint(screen_y[draw_order]).astype(np.int32)
        pixel_x = np.clip(pixel_x, 0, self.width - 1)
        pixel_y = np.clip(pixel_y, 0, self.height - 1)
        ordered_colors = colors[draw_order]

        if self.point_radius <= 1:
            image[pixel_y, pixel_x] = ordered_colors
            return image

        for point_x, point_y, color in zip(pixel_x, pixel_y, ordered_colors, strict=False):
            cv2.circle(
                image,
