    ...
    while iteration < opt.iterations + 1:
        for batch_data in training_dataloader:
            ...
            for batch_idx in range(batch_size):
                ...
                # Loss
                # Foreground-weighted L1 loss for plant-only reconstruction.
                # The plant is only ~1% of pixels, so normal full-frame L1 is dominated by black background.
                FG_WEIGHT_FOR_PLANT_PIXELS = 50.0
                if viewpoint_cam.gt_alpha_mask is not None:
                    fg_mask = viewpoint_cam.gt_alpha_mask.to(image.device)
                    weights = 1.0 + FG_WEIGHT_FOR_PLANT_PIXELS * fg_mask
                    Ll1 = (torch.abs(image - gt_image) * weights).sum() / (weights.sum() * image.shape[0] + 1e-8)
                else:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, batch_data in enumerate(tqdm(config['cameras'])):
                    ...
                    # Save evaluation render/GT/alpha/depth PNGs to disk.
                    if idx < 5:
                        save_dir = os.path.join(scene.model_path, "eval_renders", f"iter_{iteration}", config['name'])
                        os.makedirs(save_dir, exist_ok=True)
                        safe_name = str(viewpoint.image_name).replace("/", "_")
                        save_image(image, os.path.join(save_dir, f"{idx:03d}_{safe_name}_pred.png"))
                        save_image(gt_image, os.path.join(save_dir, f"{idx:03d}_{safe_name}_gt.png"))
                        save_image(alpha, os.path.join(save_dir, f"{idx:03d}_{safe_name}_alpha.png"))
                        save_image(depth, os.path.join(save_dir, f"{idx:03d}_{safe_name}_depth.png"))
