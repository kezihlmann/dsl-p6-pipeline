...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    # Plant project patch: load SAM3 masks from sibling masks/ folder.
    # Original 4DGS images are RGB/JPG, so there is no alpha channel.
    # Expected:
    #   images/GX010014_....jpg
    #   masks/GX010014_...._mask_sam3.png
    if loaded_mask is None:
        try:
            ...
            candidates = [
                mask_dir / f"{image_path.stem}_mask_sam3.png",
                mask_dir / f"{image_path.stem}.png",
                mask_dir / image_path.name,
                mask_dir / f"{image_path.stem}_mask.png",
                mask_dir / f"{image_path.stem}_mask_ground_truth.png",
            ]
            ...
            if mask_path is not None:
                ...
                loaded_mask = PILtoTorch(mask_img, resolution)
                if loaded_mask.shape[0] > 1:
                    loaded_mask = loaded_mask[:1, ...]
                loaded_mask = (loaded_mask > 0.5).float()
            else:
                print("Warning: no external SAM3 mask found for", image_path.name)
        except Exception as e:
            print("Warning: failed to load external SAM3 mask for", getattr(cam_info, "image_name", "unknown"), e)
